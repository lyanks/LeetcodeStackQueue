...
class MyStack:
    ...
    def __init__(self):
        """ init """
        self.queue_top = LinkedQueue()
        self.queue_bottom = LinkedQueue()

    def push(self, x: int) -> None:
        """ push """
        self.queue_bottom.add(x)
        while not self.queue_top.is_empty():
            node = self.queue_top.pop()
            self.queue_bottom.add(node.value)
        self.queue_top, self.queue_bottom = self.queue_bottom, self.queue_top
    ...
    def top(self) -> int:
        """ top """
        node = self.queue_top.peek()
        return node.value

    def empty(self) -> bool:
        """ empty """
        return self.queue_top.is_empty()
